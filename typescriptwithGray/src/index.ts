// it is a user defiend type it seems to be like a class or an object
// which we specify the properties and it's type
type player = {
  name: string,
  age: number,
...
const multiplay: mathFunction = (m, i) => {
  return m * i;
}
...
let myVal: string = addOrConcat(2, 5, 'concat') as string; // will concat them as string
...
let nextVal: number = addOrConcat(2, 5, 'add') as number; // will sum them as numbers
